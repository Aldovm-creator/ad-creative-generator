...
function resolveType(t?: LeadMagnetType): 'book' | 'screen' | 'checklist' | 'table' | 'chart' | 'video' {
  ...
  return 'book' // Ebook, Guía, White Paper
}

function LeadMagnetMockup({ pc, sc, w = 100, id = 'e', title = '', brand = '', type: lmType }: { pc: string; sc: string; w?: number; id?: string; title?: string; brand?: string; type?: LeadMagnetType }) {
  ...
  // ── Shared defs & shell for all types ──
  const defs = (
    <defs>
  ...
  // Shadow + pages + cover + spine shell shared across book-like types
  ...
  if (kind === 'checklist') {
    ...
    const cs = Math.round(w * 0.04) // check size
    return shell(
      <>
  ...
  // ── DEFAULT: BOOK (Ebook, Guía, White Paper) ──
  return shell(
    <>
...
function AdCanvas({ variant, data, size: S }: CP) {
  const { headline, subheadline, cta, primaryColor: pc, secondaryColor: sc, leadMagnetType: lmt } = data
      ...
      return (
        <div style={{ ...base, background: `linear-gradient(155deg, ${dk2} 0%, ${dk1} 35%, ${darken(pc, 40)} 100%)` }}>
          ...
          <div style={{
            ...
            <LeadMagnetMockup pc={pc} sc={sc} w={Math.round(ew * 1.3)} id="v1" title={headline} type={lmt} />
          </div>
        ...
        <div style={{ ...base, background: `linear-gradient(160deg, #0a0a0f 0%, ${dk3} 40%, ${dk2} 70%, #08080d 100%)` }}>
          ...
          <div style={{
            ...
            <div style={{ filter: `drop-shadow(0 8px 20px rgba(0,0,0,0.5)) drop-shadow(0 20px 40px ${rgba(pc, 0.2)})` }}>
              <LeadMagnetMockup pc={pc} sc={sc} w={Math.round(ew * 1.2)} id="v2" title={headline} type={lmt} />
            </div>
          </div>
      ...
      return (
        <div style={{ ...base, background: '#f4f5f7' }}>
          ...
          <div style={{
            ...
            <LeadMagnetMockup pc={pc} sc={sc} w={Math.round(ew * 0.9)} id="v3" title={headline} type={lmt} />
          </div>
      ...
      return (
        <div style={{ ...base, background: '#fafbfc' }}>
          ...
          <div style={{
            ...
            <LeadMagnetMockup pc={pc} sc={sc} w={Math.round(ew * 1.1)} id="v4" title={headline} type={lmt} />
          </div>
        </div>
      )
      ...
      return (
        <div style={{
          ...
          <div style={{
            ...
            <LeadMagnetMockup pc={pc} sc={sc} w={Math.round(ew * 1.05)} id="v5" title={headline} type={lmt} />
          </div>
      ...
      return (
        <div style={{
          ...
          <div style={{
            ...
            <LeadMagnetMockup pc={pc} sc={sc} w={Math.round(ew * 1.35)} id="v6" title={headline} type={lmt} />
          </div>
        </div>
      )
